...
public class ERA implements Serializable {
     ...
     * @return 返回集合列表，每个元素是集合，集合内容是pi
     */
    Set<Set<String>> verticalPartitioningFast(List<String> S, Set<Character> alphabet, int Fm) {
        Set<Set<String>> virtualTree = new HashSet<>();
        ...
        //////////////
        while (!P_.isEmpty()) {
            List<String> prefixWithoutSplitterList = new ArrayList<>(P_.size());
     ...
     * @return 返回(key包含拆分符的前缀 value前缀在串的位置)
     */
    Map<String, List<int[]>> getPrefixLocFast(List<String> S, Set<String> prefixSet) {
        Map<String, List<int[]>> prefixLoc = new HashMap<>(prefixSet.size());//<prefix, Prefix Location>
        Map<String, String> prefixMap = new HashMap<>(prefixSet.size());//<prefixWithoutSplitter, Prefix>
        for (String prefix : prefixSet) {
            String prefixWithoutSplitter = prefix.replace(SPLITTER_INSERTION + "", "").replace(SPLITTER + "", "");//去掉分割标记
            prefixMap.put(prefixWithoutSplitter, prefix);
        }
        Trie trie = Trie.builder().addKeywords(prefixMap.keySet()).build();
        for (int index = 0; index < S.size(); index++) {//遍历主串，寻找所有的pi
            Collection<Emit> prefixList = trie.parseText( S.get(index));
            for (Emit prefixInfo : prefixList) {
                String sPrefixWithoutSplitter = prefixInfo.getKeyword();
                String sPrefix = prefixMap.get(sPrefixWithoutSplitter);
                List<int[]> locList = prefixLoc.get(sPrefix);
                if (locList == null) {
                    locList = new ArrayList<>();
                    prefixLoc.put(sPrefix, locList);
                }
                locList.add(new int[]{index, prefixInfo.getStart()});//寻找prefix的起始位置
            }
        }
        return prefixLoc;
    }
